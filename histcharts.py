...
def SL():
    for market in markets: #Loop trough the stock summary
        try:
          ...
          db = pymysql.connect("database-service", "cryptouser", "123456", "cryptodb")
          cursor = db.cursor()
          cursor.execute("SELECT date, twitter_polarity, twitter_score, news_score, price, positive_tweets, negative_tweets, candle_score FROM history WHERE market='%s' ORDER BY date DESC LIMIT 30" % market)
