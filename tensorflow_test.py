...
current_price = c.get_ticker('USDT-' + currency).json()['result']['Last']
...
#currency = 'MAID'
url = ('https://poloniex.com/public?command=returnChartData&currencyPair='+'USDT_'+currency+'&start='+starttime+'&end=9999999999&period=1800') #1800
#url = ('https://poloniex.com/public?command=returnChartData&currencyPair='+'USDT_'+currency+'&start='+starttime+'&end=9999999999&period=14400')
...
df.to_csv('data/test/cryptodata'+'USDT-'+currency+'.csv',index=None)

path_to_dataset = ('data/test/cryptodata'+'USDT-'+currency+'.csv')
sequence_length = 20
...
with open(path_to_dataset) as f:
    ...
    for line in f:
        fields = line.split(',')
        vector_vix.append(float(fields[4])/100000)
...
# train the model
model.fit(X_train, y_train, batch_size=512, nb_epoch=epocha, validation_split=0.05, verbose=1)
...
with open('results/test/output_result_'+'USDT_'+currency+'.txt', 'r') as f:
    ...
    mean_price=float(last_word)*100000
    predicted_price = format(float(mean_price), '.6f')

last_word = float(last_word)*100000
...
with open('results/test/output_result_'+'USDT_'+currency+'.txt', 'a') as myfile:
...
print predicted_price, direction