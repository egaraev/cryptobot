...
import time
import math
sys.stdout = open('/root/price_tracker', 'a')
...
def summ_percent(marketname):
    ...
    cursor = db.cursor()
    market=marketname
    cursor.execute("SELECT SUM(percent_change) FROM (SELECT percent_change FROM prices WHERE market = '%s' order by id desc limit 5) prices" % market)
