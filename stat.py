            ...
            if available_market_list(summary['MarketName']):
                ...
                try:
                    ...
                    cursor.execute(
                        ...
                        (currenttime, market))
                    cursor.execute('INSERT INTO stat (market, date , current_price, ai_price, ai_direction, ha_direction_hour, ha_direction, ha_direction_daily, percent_chg, volume, candles, candle_signal_short, candle_signal_long, score, score_direction, positive_sentiments, negative_sentiments, buy_orders_sum, sell_orders_sum, buy_orders_count, sell_orders_count) SELECT market, date, current_price, ai_price, ai_direction, ha_direction_hour, ha_direction, ha_direction_daily, percent_chg, volume, candles, candle_signal_short, candle_signal_long, score, score_direction, positive_sentiments, negative_sentiments, buy_orders_sum, sell_orders_sum, buy_orders_count, sell_orders_count FROM markets WHERE market="%s"' % (market))
                    db.commit()
